...
//	List<Post> findCommentByPostId (@Param("parentId") Long parentId);
	...
	@Query(value = "select P.ID, P.BODY, P.TITLE, P.USERID, count(*) as NUMOFCOMMENTS from Post P left join Comment C on P.ID = C.POSTID where P.ID >= :startIndexId and P.ID <= :endIndexId GROUP BY P.ID", nativeQuery = true)
	List<Post> findSomePostsWithCommentCount (@Param("startIndexId") Long startIndexId, @Param("endIndexId") Long endIndexId);	
